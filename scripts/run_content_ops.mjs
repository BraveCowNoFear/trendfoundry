...
const outreachFollowupsManifest = await readJson("dist/content-outreach-followups/manifest.json", {});
...
const outreachRoutesManifest = await readJson("dist/content-outreach-routes/manifest.json", {});
...
const run = {
  ...
  contentState: {
    ...
    outreachFollowups: {
      sendReceiptCount: outreachFollowupsManifest.sendReceiptCount,
      followups: outreachFollowupsManifest.followupCount,
      dueNow: outreachFollowupsManifest.dueNowCount,
      variantCounts: outreachFollowupsManifest.variantCounts || {}
    },
    dealDesk: {
    ...
    fulfillmentQueue: {
      queueCount: fulfillmentQueueManifest.queueCount,
      preparedWaitingManualSend: fulfillmentQueueManifest.preparedWaitingManualSendCount,
      fulfilledWaitingFeedback: fulfillmentQueueManifest.fulfilledWaitingFeedbackCount,
      fulfilledSubscriptionNextDue: fulfillmentQueueManifest.fulfilledSubscriptionNextDueCount,
      needsDeliveryFix: fulfillmentQueueManifest.needsDeliveryFixCount,
      conciseReady: fulfillmentQueueManifest.conciseReadyCount,
      conciseIssues: fulfillmentQueueManifest.conciseIssueCount,
      typeCounts: fulfillmentQueueManifest.typeCounts || {}
    },
    attribution: {
      campaigns: attributionManifest.campaignCount,
      gatePassed: attributionManifest.gatePassedCount,
      manuallySent: attributionManifest.manualSentCount,
      repliesAttributed: attributionManifest.repliesAttributedCount,
      dealsAttributed: attributionManifest.dealsAttributedCount,
      fulfilledAttributed: attributionManifest.fulfilledAttributedCount,
    ...
    sendBatch: {
      rows: sendBatchManifest.batchCount,
      unsentPassed: sendBatchManifest.unsentPassedCount,
    ...
    outreachRoutes: {
      rowsWithRoutes: outreachRoutesManifest.rowsWithRoutes,
      routeCandidateCount: outreachRoutesManifest.routeCandidateCount,
      highConfidenceRoutes: outreachRoutesManifest.highConfidenceRoutes,
      sourceBatchRows: outreachRoutesManifest.sourceBatchRows
    },
    customerSuccess: {
...
- Outreach follow-ups: ${outreachFollowupsManifest.followupCount ?? "unknown"} due drafts from ${outreachFollowupsManifest.sendReceiptCount ?? "unknown"} send receipts
...
- Outreach routes: ${outreachRoutesManifest.routeCandidateCount ?? "unknown"} route candidates across ${outreachRoutesManifest.rowsWithRoutes ?? "unknown"}/${outreachRoutesManifest.sourceBatchRows ?? "unknown"} send-batch rows, ${outreachRoutesManifest.highConfidenceRoutes ?? "unknown"} high-confidence
...
19. Review \`dist/content-outreach-followups/followups.md\` for due sent-but-unreplied campaigns.
20. Review \`dist/content-deal-desk/deal-desk.md\` when a reply, invoice request, or payment confirmation arrives.
21. Review \`dist/content-fulfillment-queue/fulfillment-queue.md\` before any manual buyer delivery.
...
26. Review \`dist/content-outreach-routes/routes.md\` for public contact-route candidates; these are not permission to send.
27. Review \`dist/content-customer-success/followup-drafts.md\` after any delivered order enters \`fulfilled_waiting_feedback\`.
28. Review \`dist/content-testimonials/testimonial-bank.md\` before reusing any quote in sales copy.
29. Review \`dist/content-action-brief/action-brief.md\` for the prioritized private action queue.
30. Review \`docs/content-health-gate.md\` before trusting console-rendered Chinese text.
31. If approved, use \`dist/buyer-content-pack/delivery-email.md\` as the human-reviewed send draft.
32. If the buyer requests a custom niche, run \`npm run custom-proof-pack -- --niche="..." --platform="..." --buyer="..." --channel="..."\`.
`;
